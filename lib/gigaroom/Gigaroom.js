...
class GigaRoom extends ReadyResource {
  ...
  _registerHandlers() {
    ...
    this.router.add('@gigaroom/edit-message', async (data, context) => {
      ...
      const { canEditMessage } = context.validate;

      // Check if message can be edited
      // if (!canEditMessage(data)) {
      //   console.warn('Unauthorized message edit attempt:', data.id, data);
      //   return; // Silently reject unauthorized edit
      // }

      // Verify the message signature
      if (!this._verifyMessageSignature(data)) {
        console.warn('Invalid signature for message edit:', data.id);
        return;
      }
      ...
      // Additional checks
      if (!originalMessage) {
        console.warn('Original message not found for edit:', data.id);
        return;
      }
      if (originalMessage.sender?.toString('hex') !== data.sender?.toString('hex')) {
        console.warn('Not signed by original sender')
        return;
      }
    ...
    this.router.add('@gigaroom/delete-message', async (data, context) => {
      const { canDeleteMessage } = context.validate;

      // Check if message can be deleted
      if (!canDeleteMessage(data)) {
        console.warn('Unauthorized message deletion attempt:', data.id);
        return;
      }
      ...
      if (!originalMessage) {
        console.warn('Original message not found for deletion:', data.id);
        return;
      }
  ...
  async _apply(nodes, view, base) {
    ...
    for (const node of nodes) {
      try {
        ...
        const validationContext = this._createValidationContext(node.value);
        await this.router.dispatch(node.value, { view, base, validate: validationContext });
  ...
  _isValidNode(node) {
    return node &&
      node.value &&
      (Buffer.isBuffer(node.value) || typeof node.value === 'object');
  }

  _createValidationContext(data) {
    return {
      canEditMessage: (messageData) => {
        // Verify message can be edited
        return this._canEditMessage(messageData);
      },

      canDeleteMessage: (messageData) => {
        // Verify message can be deleted
        return this._canDeleteMessage(messageData);
      },

      canAddMember: (memberData) => {
        // Verify member can be added
        return this._canAddMember(memberData);
      }
    };
  }
  ...
  async addWriter(key) {
    ...
    const bufferKey = b4a.isBuffer(key) ? key : b4a.from(key);

    // Log detailed writer addition attempt
    console.log('Attempting to add writer:', bufferKey.toString('hex'));

    try {
      // First, update and ensure base is ready
      await this.base.update();
      await this.base.ready();

      // Check if writer already exists to prevent unnecessary re-adding
      const existingWriters = await this._getRoomWriters(this);
      const existingWriterKeys = existingWriters.map(w => w.key);

      if (existingWriterKeys.includes(bufferKey.toString('hex'))) {
        console.log('Writer already exists, skipping');
        return true;
      }

      // Append writer addition command
      await this.base.append(dispatch('@gigaroom/add-writer', {
        key: bufferKey
      }));
  ...
  async editMessage(messageId, newContent) {
    ...
    if (!this.base.writable) {
      await this.base.update()
      await this.base.ready()
      console.log('editMessage base is ready')
    }
    ...
    // console.log('Current User Public Key:', this.owner.publicKey);
    // console.log('Original Message Sender:', originalMessage.sender);

    // Check authorization - only the sender can edit
    if (!this._canEditMessage(originalMessage)) {
      console.log('Edit Authorization Check Failed');
    ...
    const updatedMessage = {
      ...
      edited: true,
      editedAt: Date.now(),
      searchableText: newContent
    };

    // More detailed signature logging
    updatedMessage.signature = this._signMessage(updatedMessage);
    console.log('Generated Signature:', updatedMessage.signature);
    ...
    // Explicit event emission with more details
    this.emit('message:edited', {
      messageId,
      content: newContent,
  ...
  _canEditMessage(messageData) {
    // Check if the current user is the original sender
    if (!this.owner || !this.owner.publicKey) {
      console.warn('No owner set for message editing');
      return false;
    }

    const currentUserKey = Buffer.isBuffer(this.owner.publicKey)
      ? this.owner.publicKey
      : Buffer.from(this.owner.publicKey, 'hex');

    const messageSenderKey = Buffer.isBuffer(messageData.sender)
      ? messageData.sender
      : Buffer.from(messageData.sender, 'hex');

    return currentUserKey.equals(messageSenderKey);
  }

  _canDeleteMessage(messageData) {
    // Similar to edit, but could include additional logic
    // e.g., checking if user is a moderator
    return this._canEditMessage(messageData);
  }

  _canAddMember(memberData) {
    // Check if current user has permission to add members
    // This could involve checking user roles or room permissions
    return this.owner &&
      (this._userHasPermission('ADD_MEMBERS') ||
        this._isRoomAdmin());
  }
  ...
  */
  _verifyMessageSignature(message) {
    ...
    try {
      ...
      // Consistent buffer conversion
      const signatureBuffer = Buffer.isBuffer(message.signature)
        ? message.signature
        : Buffer.from(message.signature, 'hex');

      const senderBuffer = Buffer.isBuffer(message.sender)
        ? message.sender
