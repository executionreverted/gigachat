  ...
  _whenWritable() {
    ...
    const check = () => {
      if (this.user.base.writable) {
        ...
        this.user.once('identity:updated', async () => {
          console.log('Identity synchronized to paired device');
          if (this.user.seed) {
            console.log('Deriving keypair from seed after pairing')
            const keys = Gigauser.deriveKeysFromSeed(this.user.seed)
            this.user.publicKey = keys.publicKey
            this.user.keyPair = {
              publicKey: keys.publicKey,
              secretKey: keys.secretKey,
              discoveryKey: keys.discoveryKey
            }
            this.user.currentDevice = {
              publicKey: this.user.publicKey,
              lastSeen: Date.now(),
              key: localKey
            }
          } else {
            console.warn('Warning: No seed available to derive keypair after pairing')
          }
        });
...
class Gigauser extends ReadyResource {
  ...
  async _open() {
    ...
    await this.addDevice(this.currentDevice)
    if (this.replicate) await this._replicate();

    // Initialize user data
    await this.setupRecoveryResponder()
    // await this.syncAllRoomWriters()
  ...
  async _refreshIdentity() {
    try {
      ...
      this.currentDevice = {
        publicKey: this.publicKey,
        lastSeen: Date.now(),
        key: this.base.local.key.toString('hex')
      }
  ...
  async _initializeRoomInstance(roomInfo) {
    ...
    try {
      ...
      const roomStore = this.store.namespace(roomInfo.roomNamespace.trim(), { keyPair: this.keyPair });
      ...
      await this.addKeysToDeviceRooms({ roomId: roomInfo.id, localKey: room.base.local.key.toString('hex') })

      // await this.syncRoomWriters(room)
  ...
  _setupRoomEvents(room) {
    ...
    const eventsToForward = [
      ...
      'categories:updated', 'roles:updated',
      'messages:updated',
  ...
  async addDevice(deviceData) {
    ...
    const existingIndex = currentDevices.findIndex(d =>
      this._safeKeyString(d.key) === localKey
    )
    ...
    await this.base.append(dispatch('@gigauser/update-devices', {
      key: this._safeKeyString(this.publicKey),
      value: JSON.stringify(currentDevices)
    }))

    this._devices = await this._refreshDevices()
    return this._devices
  }

  async addKeysToDeviceRooms(roomData) {
    if (!this.publicKey) throw new Error('Identity not created')
    const { roomId, localKey } = roomData

    if (!roomId || !localKey) {
      throw new Error('Both roomId and localKey are required')
    }

    console.log('Adding room to device: ', roomData)
    console.log('Current devices before update:', this._devices)
    ...
    await this.base.ready()
    const currentLocalKey = this.base.local.key.toString('hex')

    // Find or create the device with matching public key
    let deviceIndex = currentDevices.findIndex(d =>
      this._safeKeyString(d.localKey) === this._safeKeyString(currentLocalKey)
    )

    console.log({ deviceIndex })

    if (deviceIndex === -1) {
      // If no device found, create a new device entry
      const newDevice = {
        publicKey: this._safeKeyString(this.publicKey),
        lastSeen: Date.now(),
        key: currentLocalKey,
        rooms: []
      }
    ...
    // Ensure rooms property exists
    if (!currentDevices[deviceIndex].rooms) {
      currentDevices[deviceIndex].rooms = []
    }

    // Check if room already exists for this device
    const roomExists = currentDevices[deviceIndex].rooms.some(
      room => room.roomId === roomId && room.localKey === localKey
    )

    // Add room if it doesn't exist
    if (!roomExists) {
      currentDevices[deviceIndex].rooms.push({
        roomId,
        addedAt: Date.now(),
        localKey
      })

      console.log('Updated devices:', currentDevices)
      ...
      // Try to sync room writers
      // await this.syncRoomWriters(room)

      console.log('Added room to device', this._devices)
    }
  ...
  async createRoom(newRoomData) {
    ...
    const roomStore = this.store.namespace(roomNamespace, { keyPair: this.keyPair })
    ...
    try {
      ...
      await this.addKeysToDeviceRooms({ roomId: room.id, localKey: room.base.local.key.toString('hex') })
  ...
  async joinRoom(inviteCode, opts = {}) {
    ...
    try {
      ...
      await this.addKeysToDeviceRooms({ roomId: roomData.id, localKey: room.base.local.key.toString('hex') })
  ...
  async syncRoomWriters(room) {
    if (!room || !this.publicKey) {
      console.warn('Cannot sync room writers: Missing room or user identity');
      return false;
    }
    ...
    try {
      ...
      // Collect all potential writer keys
      const potentialWriterKeys = new Set();

      // Add current user's public key
      potentialWriterKeys.add(this.publicKey.toString('hex'));

      // Add local cores from all devices
      for (const device of this._devices) {
        if (!device.rooms || !Array.isArray(device.rooms)) continue;
        console.log({ device })

        const deviceRooms = device.rooms.filter(
          deviceRoom => deviceRoom.roomId === room.id
        );

        for (const deviceRoom of deviceRooms) {
          if (!deviceRoom.localKey) continue;

          const deviceRoomKey = b4a.isBuffer(deviceRoom.localKey)
            ? deviceRoom.localKey
            : Buffer.from(deviceRoom.localKey, 'hex');

          if (deviceRoomKey.length === 32) {
            potentialWriterKeys.add(deviceRoomKey.toString('hex'));
          }
        }
      }

      // Additionally, add the current room's local core
      if (room.base?.local?.key) {
        potentialWriterKeys.add(room.base.local.key.toString('hex'));
      }

      console.log({ potentialWriterKeys })
      // Attempt to add each unique writer key
      let writersAdded = 0;
      for (const keyHex of potentialWriterKeys) {
        const key = Buffer.from(keyHex, 'hex');

        try {
          const isWriter = await this._isRoomWriter(room, keyHex);
          if (!isWriter) {
            // Force add writer with more detailed logging
            console.log(`Attempting to add writer: ${keyHex}`);
            await room.ready()
            await room.addWriter(key);
            writersAdded++;
          }
        } catch (addError) {
          console.error(`Error adding writer ${keyHex}:`, addError);
        }
      }

      console.log(`Synchronized room writers for room ${room.id}. Added ${writersAdded} new writers.`);
      return writersAdded > 0;
    } catch (error) {
      console.error('Error in syncRoomWriters:', error);
      return false;
    }
  }
  async syncAllRoomWriters() {
    if (!this.publicKey) {
      console.warn('Cannot sync room writers: Missing user identity');
      return [];
    }

    try {
      // Force refresh devices and rooms
      await this._refreshDevices();
      await this._refreshRooms();

      // Collect all room IDs from devices and room list
      const roomIds = new Set();
      this._devices.forEach(device => {
        if (device.rooms && Array.isArray(device.rooms)) {
          device.rooms.forEach(room => roomIds.add(room.roomId));
        }
      });

      // Also include rooms from user's room list
      this._rooms.forEach(room => roomIds.add(room.id));

      // Track synchronized rooms
      const synchronizedRooms = [];

      // Sync writers for each room
      for (const roomId of roomIds) {
        try {
          const room = await this.getRoom(roomId);
          if (!room || !room.base.writable) continue;

          // Collect all local keys from devices for this specific room
          const roomLocalKeys = this._devices
            .flatMap(device =>
              (device.rooms || [])
          ...
          // Add each local key as a writer, even if current instance isn't writable
          for (const localKeyHex of roomLocalKeys) {
            try {
              const localKey = Buffer.from(localKeyHex, 'hex');

              // Force add writer if possible
              // This method should be implemented to bypass normal writer checks
              await this._forceAddWriter(room, localKey);
            } catch (keyAddError) {
              console.error(`Error adding local key ${localKeyHex} to room ${roomId}:`, keyAddError);
            }
          }

          synchronizedRooms.push(room);
        } catch (roomSyncError) {
          console.error(`Error syncing writers for room ${roomId}:`, roomSyncError);
        }
      }

      console.log(`Synchronized writers for ${synchronizedRooms.length} rooms`);
      return synchronizedRooms;
    } catch (error) {
      console.error('Comprehensive room writer synchronization error:', error);
      return [];
    }
  }

  async _isRoomWriter(room, key) {
    if (!room || !room.base || !room.base.view) {
      console.warn('Invalid room or base for writer check');
      return false;
    }

    try {
      // Convert key to consistent format (buffer or hex string)
      const targetKey = Buffer.isBuffer(key)
        ? key.toString('hex')
        : (typeof key === 'string' ? key : key.toString('hex'));

      // Query writers from the view
      const writersStream = room.base.view.find('@gigaroom/writer', {});

      // Iterate through writers to find a match
      for await (const writerEntry of writersStream) {
        const entryKey = Buffer.isBuffer(writerEntry.key)
          ? writerEntry.key.toString('hex')
          : writerEntry.key;
      ...
      // No matching writer found
      return false;
    } catch (error) {
      console.error('Error checking room writers:', error);
      return false;
    }
  }
  ...
  async _forceAddWriter(room, key) {
    await room.base.update()
    await room.base.ready()
    // Directly append a writer addition to the base
    // This bypasses normal writer checks and ensures the key is added
    await room.addWriter(b4a.isBuffer(key) ? key : b4a.from(key))
  }

  async syncPairedDeviceRooms() {
    // Refresh rooms list and devices
    await this._refreshRooms()
    await this._refreshDevices()

    // Sync writers for existing room instances
    for (const roomInfo of this._rooms) {
      try {
        // Get or initialize the room instance
        const room = await this.getRoom(roomInfo.id)

        // Sync writers for this room
        // await this.syncRoomWriters(room)

        // Optional: Force update room data
        await room.forceUpdate()
      } catch (error) {
        console.error(`Error syncing room ${roomInfo.id}:`, error)
      }
    }
  }
