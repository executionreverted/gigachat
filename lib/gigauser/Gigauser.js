...
class Gigauser extends ReadyResource {
  ...
  async addDevice(deviceData) {
    ...
    console.log('Added device: ', currentDevices)
  ...
  async addKeysToDeviceRooms(roomData) {
    ...
    // Find the device with matching local key
    const deviceIndex = currentDevices.findIndex(d =>
      this._safeKeyString(d.key) === localKey ||
      this._safeKeyString(d.key) === currentLocalKey
    )
    ...
    if (deviceIndex >= 0) {
      ...
      console.warn('Device not found for adding room key')
    }
  ...
  async syncRoomWriters(room) {
    ...
    try {
      // Force refresh devices to ensure we have the latest data
      await this._refreshDevices();
      ...
      const potentialWriterKeys = [];

      // Add local cores from all devices
      for (const device of this._devices) {
        // Ensure device has rooms and the device's rooms array exists
        if (!device.rooms || !Array.isArray(device.rooms)) {
          console.warn('Device without rooms array:', device);
          continue;
        }

        // Find rooms for this specific device
        const deviceRooms = device.rooms.filter(
          deviceRoom => deviceRoom.roomId === room.id
        );

        for (const deviceRoom of deviceRooms) {
          // Ensure we have a local key
          if (!deviceRoom.localKey) {
            console.warn('Device room without local key:', deviceRoom);
            continue;
          }

          // Convert local key to 32-byte buffer
          ...
          if (deviceRoomKey.length !== 32) {
            console.warn(`Invalid key length for room ${room.id}. Expected 32 bytes, got ${deviceRoomKey.length}`, deviceRoomKey);
            continue;
          }

          potentialWriterKeys.push(deviceRoomKey);
        }
      }

      // Additionally, add the current user's room core
      if (room.base && room.base.local && room.base.local.key) {
        potentialWriterKeys.push(room.base.local.key);
      }

      // Remove duplicates
      const uniqueWriterKeys = [...new Set(potentialWriterKeys.map(k => k.toString('hex')))];

      // Attempt to add each unique writer key
      for (const keyHex of uniqueWriterKeys) {
        const key = Buffer.from(keyHex, 'hex');

        try {
          // Ensure the key is not already a writer
          const currentWriters = await this._getRoomWriters(room);
          const currentWriterSet = new Set(currentWriters);

          if (!currentWriterSet.has(keyHex)) {
            // Force add writer, even if not writable
            await this._forceAddWriter(room, key);
            writersAdded++;

            console.log(`Added device room core ${keyHex} as writer to room ${room.id}`);
          }
        } catch (addError) {
          console.error(`Error adding device room core as writer:`, addError);
        }
      }
  ...
  async syncAllRoomWriters() {
    ...
    try {
      // Force refresh devices and rooms to ensure we have the latest data
      await this._refreshDevices();
      await this._refreshRooms();
      ...
      // Collect unique room IDs from devices
      const deviceRoomIds = new Set();
      this._devices.forEach(device => {
        if (device.rooms && Array.isArray(device.rooms)) {
          device.rooms.forEach(room => deviceRoomIds.add(room.roomId));
        }
      });

      // Also include rooms from user's room list
      this._rooms.forEach(room => deviceRoomIds.add(room.id));

      // Sync writers for each unique room
      for (const roomId of deviceRoomIds) {
        try {
          // Get the room instance
          const room = await this.getRoom(roomId);

          if (room) {
            // Perform writer sync for this room
            const syncSuccessful = await this.syncRoomWriters(room);
      ...
      console.error('Error in syncAllRoomWriters:', error);
      return [];
    }
  }
  ...
  /**
   * Get all writers for a room
   * @private
   * @param {GigaRoom} room - The room to get writers for
   * @returns {Promise<Array>} - Array of writer keys
   */
  async _getRoomWriters(room, includeMetadata = false) {
    if (!room || !room.base || !room.base.view) {
      console.warn('Invalid room or base for writers query');
      return [];
    }
    ...
    if (room.base?.activeWriters) {
      for (const writer of room.base.activeWriters) {
        if (writer?.core?.key && (!room.base.writerKey || !writer.core.key.equals(room.base.writerKey))) {
          const writerInfo = {
          ...
          if (includeMetadata) {
            try {
              ...
              let messages = [];
              try {
                const result = await room.base.view.find('@roombase/messages', {});
